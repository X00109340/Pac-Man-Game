...
public class AI : MonoBehaviour {
    ...
    //List of available paths for the ghosts
    //0 - Blocked
    //1 - Free space
	private TileManager tileManager;
    //Represents ghost controller
	...
	void Awake()
	{
		tileManager = GameObject.Find("Game Manager").GetComponent<TileManager>();
		tiles = tileManager.tiles;
        //Check if the ghosts objects are available
        if (ghost == null)
        {
            Debug.Log("game object ghost not found");
        }
        //Checking for the game tileManager object in the game.unity file
        if (tileManager == null)
        {

            Debug.Log("game object Game tileManager not found");
        }
	}
    ...
    public void AILogic()
	{
		...
		if(nextTile.occupied || currentTile.isIntersection)
		{
			...
			if(currentTile.isIntersection)
			{
				...
				if(currentTile.up != null && !currentTile.up.occupied && !(ghost.direction.y < 0)) 		dist1 = tileManager.distance(currentTile.up, targetTile);
				if(currentTile.down != null && !currentTile.down.occupied &&  !(ghost.direction.y > 0)) 	dist2 = tileManager.distance(currentTile.down, targetTile);
				if(currentTile.left != null && !currentTile.left.occupied && !(ghost.direction.x > 0)) 	dist3 = tileManager.distance(currentTile.left, targetTile);
				if(currentTile.right != null && !currentTile.right.occupied && !(ghost.direction.x < 0))	dist4 = tileManager.distance(currentTile.right, targetTile);
