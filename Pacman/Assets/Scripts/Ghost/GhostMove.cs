...
public class GhostMove : MonoBehaviour {
    ...
    //The diffrent states of the GHOSTS - Blinky, Pinky etc
	enum State { Wait, Scatter, Chase, Init, Run };
	State state;

    //This represents the starting position of the GHOSTS
    private Vector3 startingPosition;
    //This represents the amount of time the ghost has to wait 
	...
	void Start()
	{
        ...
        _toggleInterval = gameManager.scareLength * 0.33f * 0.20f;  
        //Calling the initialize ghost method
		InitializeGhost();
	}
	...
	void FixedUpdate ()
	{
		...
		if(GameManager.gameState == GameManager.GameState.Game)
        {
            ...
            //These are all the diffrent states of the ghosts - they go from Wait -> Initialise -> Scatter to their home base -> Chase Pacman -> Run away when pacman has powerup
	...
	{
        //The starting position for each ghost is retrived from the getStartingPositionAccordingToName()
	    startingPosition = GetStartPositionBasedOnName();
        // to avoid flickering animation
        waypoint = transform.position;	
        //State is set to WAIT
		state = State.Wait;
    ...
    public void InitializeGhost(Vector3 pos)
    {
        ...
        //State is set to WAIT
        state = State.Wait;
        //Initial delay is set to 4 or 5 because it depends on the music being played
        timeToEndWait = Time.time + waitLength + GUINav.initialDelay;
        InitializeWaypoints(state);
    }

    //This method was retrieved from the google link as indicated in the project upload word document
    //https://drive.google.com/drive/folders/0B23Gdj5FRrfkcERMWjdJaTlmaWc
...
10 8";
    ...
    //This method is used to represent the diffrent starting position for each of the GHOSTS
    private Vector3 GetStartPositionBasedOnName()
    {
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		{
			//Here we check to see the state of the ghost
            //If the state is RUN the pacman has eaten the ghost
      
		    if (state == State.Run)
		    {
		        Calm();
                //Initialize the ghost to the starting position
		        InitializeGhost(startingPosition);
                //Update pacman score
                pacman.UpdateScore();
		    }
		    //If the state of ghost is not running away from pacman - Destroy pacman pacman loses a life
		    else
		    {
                //Pacman loses a life
		        gameManager.PacmanLoseLife();
		    }

		}
	}

	//Diifrent State functions Wait, Chase, Scatter, Init, RunAway
    //WAIT STATE
	void Wait()
	{
	...
	void RunAway()
	{
        ...
        if(Time.time >= timeToWhite && Time.time >= _timeToToggleWhite)   ToggleBlueWhite();
		...
		else GetComponent<AI>().RunAwayLogic();
    ...
    //Retrieved from: google drive link as specified above
	public void Frighten()
	{
		state = State.Run;
		_direction *= -1;

        timeToWhite = Time.time + gameManager.scareLength * 0.66f;
        _timeToToggleWhite = timeToWhite;
